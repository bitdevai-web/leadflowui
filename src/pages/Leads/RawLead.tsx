...
import { MOCK_RAW_LEADS, MOCK_TAGS } from "../../data/mockData";
...
export default function RawLead() {
  ...
  const data = apiData ?? { data: MOCK_RAW_LEADS, total: MOCK_RAW_LEADS.length };
  ...
  const { mutate: leadVerify } = useMutation({
    mutationFn: verifyEmail,
    mutationKey: ["verify-lead"],
    onSuccess(data) {
      toast.success("Verification Started");
      localStorage.setItem("verify-process-id", `${data.data.id}`);
      restartVerify();
    },
    onError(err) {
      toast.error(err.message);
  ...
  const { data: verificationProcess, refetch: restartVerify } = useQuery({
    queryKey: ["verification-process"],
    queryFn: () => getExecution("verify-process-id"),
    refetchInterval: 5000,
    enabled() {
      return !!localStorage.getItem("verify-process-id");
    },
  });
  ...
  }, [execution?.status, verificationProcess?.status, emailProcess?.status]);
    ...
    },
    {
      ...
      selector: (row): any => (
        <div className="flex flex-row items-center gap-1.5 py-2">
          <button
            title="Verify Email"
            onClick={() => leadVerify([row.id])}
            className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-medium bg-blue-50 text-blue-600 hover:bg-blue-100 transition-colors"
          >
            <i className="fa-solid fa-circle-check text-[11px]"></i> Verify
          </button>
          <button
  ...
  const handleVerification = () => {
    if (selected.length == 0) {
      return toast.error("Please select Leads to Verify Email");
    }
    leadVerify(selected);
  };
  ...
  return (
    <>
      <PageMeta title="Leadflow - Raw Leads" description="" />
      <PageBreadcrumb
        pageTitle="Raw Leads"
        pageDetails="Raw Leads for Further processing"
        component={
          <div className="flex flex-row gap-2">
            <Button
              isLoading={execution?.status === "RUNNING"}
              onClick={() => setModalState(ModalState.LEAD)}
              size="sm"
            >
              Generate Lead
            </Button>
            <Button
              isLoading={verificationProcess?.status === "RUNNING"}
      ...
      />
      <Loading
        isOpen={execution?.status === "RUNNING"}
        title="Generating Leads"
        subTitle="Please wait..."
      />
      <Loading
        isOpen={verificationProcess?.status === "RUNNING"}
